...
class PendingProposalWorkerView(APIView):
    def get(self, request):
        token = request.COOKIES.get("token", None) 
        verification, payload = verify_access_token(token) 
        if verification:
            if payload['role'].lower() == "worker":
                proposalObj = Proposal.objects.filter(worker_id = payload['user_id'], status="applied").select_related('job')
                serializer = ProposalApplyWorkerSerializer(proposalObj, many = True,  context = {"request":self.request})
                return Response(serializer.data, status=status.HTTP_200_OK)
...
class PastProposalWorkerView(APIView):
    def get(self, request):
        token = request.COOKIES.get("token", None)
        verification, payload = verify_access_token(token) 
        if verification:
            if payload['role'].lower() == "worker":
                proposalObj = Proposal.objects.filter(worker_id = payload['user_id']).exclude(status="applied")
                serializer = ProposalApplyWorkerSerializer(proposalObj, many = True,  context = {"request":self.request})
                return Response(serializer.data, status=status.HTTP_200_OK)
...
class ProposalDeleteView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        verification, payload = verify_access_token(token) 
        if verification:
            obj = Proposal.objects.filter(id = kwargs['id'], worker_id = payload['user_id'])
            if len(obj) == 0:
                return Response({'msg':'Proposal Not Found'}, status=status.HTTP_404_NOT_FOUND)
            obj.delete()
            return Response({'msg':'Proposal Deleted Successfully'}, status=status.HTTP_200_OK)
        return Response({"msg":"Login first"}, status=status.HTTP_401_UNAUTHORIZED)